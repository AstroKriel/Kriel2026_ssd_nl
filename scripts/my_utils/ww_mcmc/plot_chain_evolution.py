  ...
  def plot(self):
    ...
    for param_index in range(num_params):
      for walker_index in range(num_walkers):
        self.axs[param_index].plot(raw_chain[:, walker_index, param_index], alpha=0.3, lw=0.5)
      p16, p50, p84 = numpy.percentile(raw_chain[:, :, param_index], [16, 50, 84])
      self.axs[param_index].axhline(p50, color="black", linestyle="-")
      self.axs[param_index].axhline(p16, color="black", linestyle="--")
      self.axs[param_index].axhline(p84, color="black", linestyle="--")
      self.axs[param_index].set_ylabel(labels[param_index])
